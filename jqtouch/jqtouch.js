...
(function ($) {
    $.jQTouch = function (options) {
        ...
        function addPageToHistory(page, animation, state) {
            _debug();
            hist.unshift({
                page: page,
                animation: animation,
        ...
        function doNavigation(fromPage, toPage, animation, goingBack, histFrom, histTo) {
            _debug();
            ...
            var state; // inject state using custom page events. note that only one event per page is supported
            $(toPage).trigger('navigating',
                    [fromPage, toPage, goingBack, histFrom ? histFrom.state : undefined, histTo ? histTo.state : undefined, function (s) { state = s; } ]);
            ...
            function navigationEndHandler(event) {
                ...
                //$(toPage).trigger('navigated',
                //    [fromPage, toPage, goingBack, histFrom ? histFrom.state : undefined, histTo ? histTo.state : undefined]);
                ...
                if (goingBack) {
                    ...
                    addPageToHistory(currentPage, animation, state);
                }
        ...
        function goBack() {
            ...
            if (doNavigation(from.page, to.page, from.animation, true, from, to)) {
                return publicObj;
            } else {
        ...
        function goTo(toPage, animation, reverse) {
            ...
            if (doNavigation(fromPage, toPage, animation, false, hist[0], null)) {
                return publicObj;
            } else {
        ...
        function hashChangeHandler(e) {
            ...
            if (location.hash === hist[0].hash) {
                ...
                if (location.hash === hist[1].hash) {
                    //This is a hack to fire event when the hardware back button is pushed
                    $(window).trigger('goingback', [hist[0].hash, hist[1].hash]);
                    goBack();
                } else {
        ...
        $(document).ready(function () {
            ...
            addPageToHistory(currentPage, undefined, jQTSettings['initialState']);
            scrollTo(0, 0);
